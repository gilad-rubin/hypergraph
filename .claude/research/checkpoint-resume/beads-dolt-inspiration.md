...
## 1. Beads (bead-project/bead, ceumicrodata/bead, bead.zip)

### Core Model

A **bead** is a self-contained computational snapshot: `output = code(*inputs)`. It packages code, data, and dependencies into an immutable ZIP archive.

**Primitives**:
- **Workspace**: Mutable working directory during active development (code, inputs, outputs, temp)
- **Archive**: Immutable `.zip` with timestamp in filename (e.g., `analysis_20250730T120000.zip`)
- **Box**: Storage directory for completed beads (append-only collection)
- **Kind**: Human-readable conceptual identity shared across versions (e.g., `"survey-data"`)
- **Content verification**: Cryptographic hash per file in a manifest (`meta/` directory)

A bead archive contains:
```
code/          # Source scripts, notebooks — the transformation
data/          # Output files (renamed from output/ on save)
meta/          # Manifest with file hashes, bead spec, input references
input/         # References to upstream beads (kind + timestamp, verified by hash)
```

### Position vs Data

Beads does **not** track execution position at all. It tracks only the input/output DAG — which beads depend on which other beads. There is no concept of "where you are" in a pipeline; the system is purely about capture and verification.

**Key insight**: Beads separates "what was computed" (the archive) from "what is being computed" (the workspace). This maps directly to our `WorkflowStatus.ACTIVE` vs `WorkflowStatus.COMPLETED` distinction. A workspace is an active run; an archive is a completed checkpoint.

### Schema / Structure Changes

When the transformation code changes, you create a **new bead** with the same `kind`. The old bead remains immutable. Downstream beads are NOT automatically invalidated — the user must explicitly update inputs (`bead input update`) and re-run.

**No automatic cascade**: If A -> B -> C and A changes, B and C still reference the old A. You manually `bead edit B`, update its input to new A, re-run, save as new B, then repeat for C.

This is the conservative end of the spectrum: total immutability, manual propagation.

### Fork / Branch Semantics

Forking is implicit through the DAG structure:
- Create a new bead that references the same inputs as an existing one but with different code = a "variant"
- Multiple beads can share the same input bead (fan-out)
- The DAG visualizer (`bead web`) shows the full dependency graph

There is no explicit branch/merge concept. The DAG is append-only: new beads are added, old beads are never modified.

**What's shared vs copied**: Input data is loaded by reference (verified by hash). Code and outputs are physically stored in each archive. No structural sharing — each bead is a complete snapshot.

### Inspiration for Hypergraph

**1. Kind vs Content ID = Workflow Definition vs Run**

The `kind` / `content_id` pattern maps cleanly to our model:

| Beads | Hypergraph |
|-------|------------|
| `kind` (shared across versions) | `graph_name` / graph identity (same DAG structure) |
| content hash (unique per version) | `workflow_id` + `graph_hash` (specific run of specific graph version) |
| Timestamped archive name | `run_id` with `created_at` |

This reinforces our `graph_hash` design: the hash identifies the graph structure (like `kind`), while the `workflow_id` identifies the specific execution (like the content hash).

**2. Workspace -> Archive = Active -> Completed Transition**

The workspace/archive lifecycle is a clean metaphor for run status:

```
Workspace (mutable, in-progress)  →  bead save  →  Archive (immutable, verified)
     ↕                                                    ↕
Run(status=ACTIVE)                →  completion  →  Run(status=COMPLETED)
```

The key behavioral difference: once a bead is archived, it's done. You don't "resume" an archive — you `bead edit` to create a new workspace from it. This supports the spec's position that COMPLETED runs should error on re-run: if you want to continue, you fork (create a new workspace from the checkpoint), not mutate the completed run.

**3. Manifest = Step History as Integrity Check**

Beads uses file-level hashes in a manifest to verify that loaded inputs match what was originally computed. We should do the same for step history: when loading a checkpoint, verify that the stored `graph_hash` matches the current graph. The manifest concept maps to our `graph_hash` check.

**4. Explicit Dependencies, Not Inferred**

Beads requires explicit `bead input add` — dependencies are declared, not discovered. Our checkpoint system similarly should be explicit about what it stores and loads. The current "silently filter to `graph.inputs.all`" approach is the opposite of this philosophy. Beads would say: store everything, verify on load, fail loudly on mismatch.

**5. No Replay, Only Snapshot**

Beads does not replay computations. It captures snapshots. This aligns with our "Restore + Continue, not Replay" approach. We don't re-execute completed nodes — we restore their output values and let the engine figure out what to run next.
