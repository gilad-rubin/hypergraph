...
Nest graphs as nodes (`.as_node()`) instead of adding flags or config surfaces. **Nested graphs are first-class citizens** — every framework feature (checkpointing, debugging, observability) must work with them. That's what "first-class" means.
