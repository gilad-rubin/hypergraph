...
| 2 | `call_llm` | `llm_response` = AI("The answer is 7.") |
| 2 | `add_response` | `messages` v3 = [System, Human, AI, Tool, AI] |
| 2 | `has_tool_calls` | False → END |

**Final result:** `messages` = complete conversation (including system prompt)
...
| `add_edge("a", "b")` | Implicit from param names |
| `Annotated[list, operator.add]` | Explicit accumulator nodes |
| `add_conditional_edges(...)` | `@branch(when_true=..., when_false=...)` |
