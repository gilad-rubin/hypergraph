...
on from outside ``hypergraph.graph`` and carry a compatibility contract:

- ``describe_addressed_input`` and ``flatten_subgraph_addressing`` are imported
  by ``hypergraph.runners._shared.value_resolution``,
  ``hypergraph.runners._shared.input_normalization`` and
  ``hypergraph.runners._shared.validation``; ``get_edge_produced_values`` is
  imported by ``hypergraph.runners._shared.validation``.
- ``graph/core.py`` and ``graph/input_spec.py`` use the same definitions, which
  is the point: the runner boundary and ``Graph.bind`` must agree.

Their output is user-visible. ``describe_addressed_input``'s
``"'x' of subgraph 'inner'"`` shape is inlined verbatim into missing-input
errors and bind-override warnings, and ``flatten_subgraph_addressing``'s
``ValueError`` messages surface to users as-is. Changing either format changes
what users read, so treat both as public behavior, not as an implementation
detail (see ``dev/CODE-CONVENTIONS.md`` § Module Naming: a cross-module
internal API does not carry a ``_`` prefix).
"""
