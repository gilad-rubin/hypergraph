...
from hypergraph.runners._shared.run_teardown import InspectionSettlement, RunTeardown
    ...
    def run(
        ...
        **input_values: Any,
        ...
        try:
            if sync_cp is not None and _validation_ctx is None:
                ...
                if resume_checkpoint is not None:
                    ...
                    retention_policy = getattr(sync_cp, "policy", None)
                    read_raw_steps = getattr(sync_cp, "steps", None)
                    if source_run_id is not None and getattr(retention_policy, "retention", "full") != "full" and callable(read_raw_steps):
                        validate_restorable_history(
                            graph=graph,
                            steps=read_raw_steps(source_run_id, superstep=resume_checkpoint.source_superstep, show_internal=True),
        ...
        teardown = RunTeardown(
            ...
            shutdown_dispatcher=self._shutdown_dispatcher_sync,
        ...
        try:
            ...
            try:
                dispatcher = teardown.settle(dispatcher)
            except BaseException as final_error:
