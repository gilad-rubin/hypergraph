...
def traverse_to_leaves(
    ...
    Example:
        >>> # Expand only to depth 1
        >>> def max_depth_1(G, node_id, depth):
        ...     return depth < 1
        >>> G = nx.DiGraph()
        >>> G.add_node('a', parent=None)
        >>> G.add_node('b', parent='a')
        >>> G.add_node('c', parent='b')
        >>> traverse_to_leaves(G, 'a', max_depth_1)
        ['b']
    """
    leaves: list[str] = []
...
def _traverse_recursive(
    ...
    Args:
        ...
        should_expand: Expansion predicate
        depth: Current depth from root
        leaves: Accumulator list for leaf nodes
    """
