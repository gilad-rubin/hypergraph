...
Each card renders its call as written, so inputs follow visualize()'s
defaults (hidden, shown as ghost pills on hover; bound inputs included)
unless the call sets show_inputs / show_bounded_inputs. Every card lists the
input mode it renders.
"""
...
class VizCapture:
    ...
    def visualize(self, graph: Any, *args: Any, **kwargs: Any) -> Any:
        ...
        # Record the input mode the card renders, defaults included, so every card states it.
        from hypergraph.viz.widget import _resolve_input_visibility

        recorded = dict(kwargs)
        recorded["show_inputs"], recorded["show_bounded_inputs"] = _resolve_input_visibility(
            kwargs.get("show_inputs"), kwargs.get("show_bounded_inputs"), kwargs.get("show_external_inputs")
        )
        self._record(graph, filepath, recorded)
        self._counter += 1
        return result
...
def build_index(output_dir: Path, records: list[VizRecord], *, iframe_height: int) -> Path:
    ...
    lines = [
        ...
        "    <span class='title'>Dagre Layout</span>",
        "    <div class='group'>",
        ...
        "  <div class='meta'>Generated by scripts/render_notebook_viz.py. Each card renders its notebook call as written:"
        " inputs follow visualize()'s defaults (hidden; hover a step for its ghost pills, or press Show Inputs)"
        " unless the call sets show_inputs, and each card lists its input mode.</div>",
    ]

    esc = html_mod.escape
    for notebook, items in records_by_notebook.items():
        lines.append(f"  <h2>{esc(notebook)}</h2>")
        for item in items:
            ...
            label = esc(item.graph_name or f"graph {item.index}")
