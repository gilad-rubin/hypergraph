...
**Routed reconcile** — the exception to settling one derived column at a time. When the node a reconcile reaches is routed by a gate, it cannot be run on its own: which branch runs is the gate's decision, not the planner's. The planner runs the gate's whole slice — the gate and everything downstream of it — in one go, and settles every node that slice covers from that single run, without asking the runner again. A node the gate routed away from did not run and keeps what the row already had; where its column has several producers (a union column), the stamp already recorded for that column wins, rather than restamping it with a branch that produced nothing — except, today, for the very node the reconcile was reached for, which is settled with its own provenance and can be restamped with a branch that did not run (a pinned defect, tracked in #481). The slice is cut from the graph the row belongs to, so a gate inside a `map_over` child graph reconciles a child row from the child's graph, not the root's.

**Row fingerprint** — a row-level hash `hash(all source content-key values + all node definition hashes + all bound-value payloads)`. Used as a fast path: if unchanged, no per-column provenance checks are needed. Incorporates the full derivation plan — source values, node code, component configs, and bound plain values — so swapping a component invalidates the fingerprint even when source values are unchanged. Bound values count **at every depth**: a value bound on a graph mounted as a `GraphNode`, or on a graph nested inside that one, parameterizes derivation exactly like a root binding and moves the fingerprint the same way. One precedence rule settles a name bound at more than one depth, in identity and in execution alike — **the outermost binding wins**, because an outer bind flattens down through the boundary at run time; the inner value it shadows is not recipe and never reports drift. The mirror of that rule on the input side: a name bound anywhere in a node's subgraph is never a run value for that node, and therefore never a stored source column either — a child table builds no column for a name its child graph binds. (A store written before that rule keeps its dead column on disk; the read and rebuild paths ignore it, so no migration is needed and the child fingerprint does not depend on how the row was produced. "Ignore" is narrow on purpose: only a name that is bound, that the spec declares no column for, and whose stored value is NULL. A column the spec declares, or a user annotation written under that name before the graph grew the binding, holds real data and still reads back — it just cannot be written under that name any more.) Parent and child rows each have their own fingerprint scoped to their respective graph; the parent's additionally carries the mounted child graphs' bound-value payloads, so a child-only recipe change cannot leave the parent row skipped while its children need re-deriving.
...
**Identity** — the stable, user-facing key for a row. Declared explicitly on the table (`identity="video_id"`) and at each grain boundary (`map_over(..., identity="utterance_id")`). Used for update, delete, sync matching, and parent-child links. Always explicit — no naming-convention magic. A child identity must be unique within one parent: the child key is `(parent identity, child identity)`, so a write whose mapped items share an identity under one parent is REFUSED with `DuplicateChildIdentityError` before any child graph runs and before any child row of that parent is written — every child table of the row checked together; under `on_error="store"` the parent becomes an error row. An item without the identity field has the empty identity. A child table is named after its child identity, so two `map_over` boundaries whose identities resolve to one table name, or one that resolves to the root table's name, are refused at table analysis with `GraphConfigError`; so is a root or child table that resolves to the recipe journal's reserved table name (`recipe_journal`).
...
**Error message (`_error`)** — an internal column storing `None` (success) or `"{ExceptionType}: {message}"` (failure). Only populated when `on_error="store"` and derivation fails — on an error row it is the failure that ended the row, on a partial row the first failure the run could attribute to a node.
...
**Partial row** — a row written under `on_error="store"` when the runner could blame the failure on a node: the derived columns the run produced are stored with their provenance stamps, every column the run did not produce is `None` with a recorded reason, and `_status="partial"`. It counts as stale in `status()`, never fresh, and the next `sync()` re-derives exactly the null columns through the column-scoped reconcile path — the columns it kept are not paid for twice. A partial row names every node of its graph that failed with a `node_error` change. A failed fan-out boundary also leaves a partial row: the parent stores no column for the mapped items, so the columns that succeeded stand, the earlier child rows stay as they are, and the next `sync()` re-runs only the boundary — unless the boundary also produces a stored parent column, in which case the whole graph runs again for the row. A failure the runner cannot attribute to a node, one that leaves no derived column standing, or one on a top-level node of this table's graph that owns no stored column and is not a fan-out boundary (nothing a column change could name, nothing a column-scoped heal would run again) is an error row instead: a row never claims column granularity the run cannot support.

**Column change (`_changes`)** — an internal column on a partial row: one JSON entry per nulled column recording the column, the node, and a reason code. `node_error` means that column's own producer raised (the entry carries its message); `upstream_error` means a failed node reaches that producer through the graph, so its inputs never arrived; `not_run` means nothing on the column's own path failed and the run simply ended before its producer was scheduled. A column whose producer ran and returned `None` gets no entry — that is a value, not a failure. A failed fan-out boundary gets one `node_error` entry per `map_over` input, however many child tables map over it, naming that input instead of a stored column (so the name is not a key of the row), with the boundary as the node. Read as typed `ColumnChange` values through `table.partial()`.
...
5. **Grain boundary** — when a `map_over` node is encountered, a child table is created. The child's identity comes from `map_over(..., identity=...)`. The child's source columns come from the child graph's inputs (required plus defaulted), minus the identity, the bound components, and any name the child graph itself binds at any depth — a bound name is recipe, not data. The child's derived columns come from the subgraph's nodes (analyzed recursively).
