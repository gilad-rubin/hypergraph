...
- R20 (#451/#484, foreman 2026-09-20, pending maintainer ratification): `map_iter(workflow_id=)` is identity only — item i runs as `<workflow_id>/<i>` and nothing else moves (no batch row, no resume, no `_parent_run_id`, no span change); the option is refused with the existing `ValueError` when a checkpointer is attached, because a durable map is `runner.map(..., workflow_id=...)`.
- R21 (#452/#496, foreman 2026-09-20, pending maintainer ratification): `start_refused` is reserved for deterministic pre-run refusals — the `ValueError` family, `MissingInputError`, the restore guards and `IncompatibleRunnerError`; a submission that fails once a runs row exists is a failed run, not a dead letter, and the reason keys on "no runs row".
- R22 (#462/#463 → #493, foreman 2026-09-20, pending maintainer ratification): a lost `ctx.record` fact fails the node on every runner x body pairing; a body's try/except around `ctx.record` changes only the body's own control flow and cannot convert a dropped fact into a completed run.
