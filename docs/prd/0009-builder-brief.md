...
changing one argument/keyword changes identity and opaque state refuses.

Expected touch surface is `src/hypergraph/_utils.py`, `tests/test_utils.py`, and
the PRD status. Reuse `hash_definition()` recursively for `partial.func` and the
existing `_canonicalize()` for arguments. Exact-type check partials; do not add a
second normalizer or change Graph/node/runner APIs.
