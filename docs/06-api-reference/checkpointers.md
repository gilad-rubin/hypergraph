...
| `AttemptError` | `type_name`, `message` | Privacy-safe error projection: exception type identity only. No args, no stack traces, no `repr` of user values, and no `str(exc)` message text — `message` stays empty for framework-created records (kept for schema stability). The exact exception object remains on local surfaces at failure time; see [Errors — the privacy boundary](errors.md#the-privacy-boundary). |
